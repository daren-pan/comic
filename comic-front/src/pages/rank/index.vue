      ...
      <template v-else>
        ...
        >
          <h3 class="block-head">
            ...
            <a href="javascript:;" class="block-more" @click="router.push({ path: '/search', query: { category: cat.name } })">查看全部 ›</a>
          </h3>
          ...
          <ol v-else class="rank">
            <li v-for="(c, i) in lists[cat.name]" :key="c.id" class="item">
              ...
              <a href="javascript:;" class="thumb" @click="router.push(`/comic/${c.id}`)">
                <img :src="c.cover" :alt="c.title" loading="lazy" />
              </a>
              <div class="info">
                <a href="javascript:;" class="name" @click="router.push(`/comic/${c.id}`)">{{ c.title }}</a>
...
.thumb { flex: 0 0 auto; width: 40px; aspect-ratio: 3 / 4; border-radius: 6px; overflow: hidden; background: #eee; text-decoration: none; }
