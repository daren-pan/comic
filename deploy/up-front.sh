#!/usr/bin/env bash
# ============================================================
#  一键：构建 comic-front(H5) 前端产物 + 后端 wheel → 镜像 → 启动整套服务 → 自检
#
#  与 deploy/up.sh 的唯一区别是**前端来源**：本脚本用 comic-front（uni-app）的 H5 构建，
#  其余流程（代码更新 / 前置检查 / 镜像构建 / 起服务 / 自检）完全一致。
#
#  依次做六件事：
#    0) 更新代码（`git pull`，只快进不合并；--skip-pull 跳过）
#    1) 前置检查（docker / compose v2 / deploy/.env / 运行时数据目录）
#    2) 构建前端产物：cd comic-front && npm run build:h5  →  comic-front/dist/build/h5
#    3) 调 deploy/build.sh（用 WEB_SRC 把前端来源指到上面的产物）：生成 wheel、复制前端产物，
#       按序构建 5 个镜像（mysql → web → crawler → api → nginx）
#    4) docker compose up -d
#    5) 自检：mysql 健康 → 容器内接口可用 → 数据目录可写 → 对外入口 HTTP 码 → 前端身份
#
#  用法：
#    bash deploy/up-front.sh              # 全量：更新代码 + comic-front build + 镜像 build + 起服务
#    bash deploy/up-front.sh --skip-web   # 前端没改，跳过 npm build（快很多）
#    bash deploy/up-front.sh --skip-pull  # 不更新代码，直接按当前工作区构建
#    bash deploy/up-front.sh --collect    # 额外启动定时采集（comic-scheduler）
#    bash deploy/up-front.sh --migrate    # 起完服务后，对**已有库**跑一遍幂等迁移脚本
#    bash deploy/up-front.sh -h
#
#  说明：
#    · 脚本是**幂等**的 —— 重复跑就是更新代码 + 重新构建 + `up -d`，不会清数据；
#      真正会丢数据的只有 `docker compose down -v`（删数据库卷）与手动删数据目录。
#    · **前端是「替换」不是「并存」**：comic-front 的 H5 产物被复制进 deploy/web/dist/，
#      再由 comic-api 镜像烘进去、在 `/` 同源托管 —— 与 comic-web 走的是**同一个位置**。
#      所以跑过本脚本后站点就是 comic-front；再跑 `deploy/up.sh` 会切回 comic-web。
#    · 前端产物是烘进 comic-api 镜像的（COPY --from=comic-web），所以改前端必须重跑本脚本。
#    · H5 产物用**相对路径**引用资源（manifest 里 h5.router.base = "./"）、路由是 hash 模式，
#      所以放在站点根目录直接就能跑，不需要额外配 base 或 history 回退。
# ============================================================
set -euo pipefail
...
#    这也是 build.sh / up.sh 里一律用相对路径的同一个原因。
compose() { docker compose -f docker-compose.yml "$@"; }

# 前端产物目录：comic-front 的 H5 构建输出（可用 WEB_DIST=<目录> 覆盖）
WEB_DIST="${WEB_DIST:-$ROOT/comic-front/dist/build/h5}"
case "$WEB_DIST" in
  /*) ;;                                      # POSIX 绝对路径
  [A-Za-z]:*) ;;                              # Windows 盘符（C:/ 或 C:\）
  *) WEB_DIST="$ROOT/$WEB_DIST" ;;            # 相对路径按仓库根解析
esac
...
if [ "$SKIP_PULL" = "1" ]; then
  ...
  if PULL_OUT="$(git -C "$ROOT" pull --ff-only 2>&1)"; then
    ...
    if [ "$BEFORE" = "$AFTER" ]; then
      ...
      # up-front.sh / up.sh / build.sh 本身也在仓库里：这次拉下来的新版本要下一次跑才生效
      if git -C "$ROOT" diff --name-only "$BEFORE" "$AFTER" | grep -qE '^deploy/(up|up-front|build)\.sh$'; then
        echo "   ⚠️ 这次更新动了 deploy/ 下的脚本 —— 当前跑的还是旧脚本，建议再跑一次"
      fi
    fi
  else
    echo "   ⚠️ git pull 失败 —— 按当前工作区继续构建。git 原话如下："
    printf '%s\n' "$PULL_OUT" | sed 's/^/        /'
    echo "        （想丢弃本地改动后重跑：git -C '$ROOT' checkout -- . && bash deploy/up-front.sh）"
  fi
fi
...
# ---------- 2. 前端（comic-front → H5） ----------
step "[2/6] 前端产物（comic-front → H5）"
if [ "$SKIP_WEB" = "1" ]; then
  [ -d "$WEB_DIST" ] || die "--skip-web 但 $WEB_DIST 不存在，去掉该参数重跑"
  echo "   跳过（--skip-web），复用现有 $WEB_DIST"
else
  ...
  [ -d "$WEB_DIST" ] || die "前端构建后仍然没有 $WEB_DIST"
fi
echo "   产物: $(find "$WEB_DIST" -type f | wc -l | tr -d ' ') 个文件"

# ---------- 3. 产物 + 镜像 ----------
step "[3/6] 生成 wheel/产物 + 按序构建 5 个镜像（mysql → web → crawler → api → nginx）"
# build.sh 默认取 comic-web/dist；这里用 WEB_SRC 把它换成 comic-front 的 H5 产物。
# ⚠️ 先把旧的前端产物清掉：build.sh 的复制是「合并覆盖」语义，不清会把两个前端的
#    assets 混在一个目录里（comic-web 与 comic-front 的 hash 文件名不同，会同时留着）。
rm -rf "$DEPLOY/web/dist"
WEB_SRC="$WEB_DIST" bash "$DEPLOY/build.sh"
...
if command -v curl >/dev/null 2>&1; then
  ...
  # 确认烘进去的确实是 comic-front 的 H5 产物：uni 的 index.html 带 <!--app-html--> 占位注释，
  ...
  case "$HTML" in
    ...
    *'id="app"'*) echo "   前端产物: ⚠️ comic-web —— 本次应部署 comic-front，检查是否用了 --skip-web 复用了旧产物" ;;
    "")           : ;;
...
cat <<EOF
  ...
  入口（宿主 HTTP_PORT=$PORT）—— 本次站点是 **comic-front**（uni-app H5）
    站点        http://127.0.0.1:$PORT/
    管理台      http://127.0.0.1:$PORT/#/pages/admin/index       （需管理员登录）
    授权页      http://127.0.0.1:$PORT/#/pages/admin/users       （给其他用户授权）
    日志页      http://127.0.0.1:$PORT/#/pages/admin/logs
  ...
  常用命令
    更新部署  bash deploy/up-front.sh   # comic-front 前端 + 后端（先 git pull 再重建）
    切回 web  bash deploy/up.sh         # 前端换回 comic-web（同一个站点位置）
