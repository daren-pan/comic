...
- `heal_covers` —— 封面自愈：外链未落盘 / 本地文件缺失的封面按状态修复。
  **由管理台「触发巡检」在 job 里接着调用**（原挂在已删除的「触发转存」上，2026-09-21 并入巡检），
  并透传触发时的 `source` 以便只自愈该源（None = 全库）。
  ⚠️ 定时巡检（`scheduler` 的 `inspect_sync`）**不含本步** —— 只有管理台那次手动巡检才调，
  避免每小时多打源站请求。
...
def heal_covers(
    ...
    """封面自愈（管理台「触发巡检」的 job 里自动执行）：修复图库中缺失/未落盘的封面。

    `source`：只自愈该数据源的封面；None = 全部源。
    管理台「触发巡检」会透传所点的源，使这次任务的**转存与封面自愈都只作用于同一个源**
