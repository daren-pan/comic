...
- **`last_updatetime`（Unix 秒级时间戳）是增量窗口过滤字段** → 适配器解析成
  `ComicBrief.source_updated_at`
- 无明确 `has_next` 标志：以「本页满 20 部 && page < MAX_PAGE」判断；增量模式再叠加
  「本页无窗口内作品即停」（翻页至时间窗口边界，防遗漏批量更新）
...
- 图源自带秒级 `last_updatetime`，故支持**精确到秒**的窗口过滤：
  只保留 `source_updated_at > since`（严格大于，保证边界不含已同步的）
- 受控限制：接口仅暴露最近更新榜、无当日翻页能力，增量模式下 `MAX_PAGE=1`
  （只扫首页最近更新 20 部）
...
- 每章关键字段：`chapter_id`（源站章节ID）、`chapter_name` / `chapter_title`（标题）、
  `chapter_order`（**全局有序整数，每部内唯一，最可靠的排序/唯一键**）
...
- ⚠️ 签名**短时效**（约数日过期）：采集入库的 page_url 只是快照，批量晚转存会 403。
  懒转存已内置兜底：本地解析 URL `t` 预判过期，过期或下载失败时经适配器
  `fetch_source_page_urls` 现场重新请求本接口重签 URL 再下载；无需手动重采。
...
- **⚠️ 详情接口的逐章 `canRead` 不可靠，别拿它判不可读**：该字段是未计算的默认值，
  会整篇为 `false`，而章节接口对可读章节正常返回 `canRead=true` 与 `page_url`。
...
- **源站可能部分章节没有数据**（`page_url` 为空，与收费/限免无关）。
  接口层面**无法区分「数据缺失」与「需付费」**，因此不做归因，只按「能否取到图」处理：
  只要**任一章**能取到图就允许导入，取不到的章节届时回落占位图。
- **图片域名白名单 `image_hosts = {"images.zaimanhua.com"}`**：正文图与封面都在该图床域
  （带 `sign+t` 短时效签名）。声明后，转存与读时穿透取图只允许下载该域，
  防止库内 URL 被当作任意请求跳板。

## 4. 章节号（chapter_no）取 key

- **主取**：源站 `chapter_order`（全局有序整数，精确区分分卷小话，如 `153.5话=1680` / `153话=1670`）
- **回退**：仅在 `chapter_order` 缺失/非法时用 `_chapter_no()` 正则解析（只认整话
  `第128话`→128；含小数点 `第153.5话` **拒绝解析返回 None**，避免误读成 `5`）
