...
- **封面取原图**：源站封面 URL 带 `.328x422.jpg` 缩略后缀，去掉即原图
  ...
  ⚠️ **剥离后要留原图自己的扩展名**：URL 形如 `{id}.{原图扩展名}.{缩略规格}.{缩略图格式}`，
  缩略图**恒输出 `.jpg`**，所以 `1788335868.jpeg.328x422.jpg` 的原图是 `1788335868.jpeg`
  而不是 `….jpg` —— 照搬末段会 404（2026-09-18 修，实测 `1789615473.png` → 200 / `….jpg` → 404）。
