...
- **封面取原图**：源站封面 URL 带 `.328x422.jpg` 缩略后缀，去掉即原图
  （实测 77KB → 1.27MB），适配器 `_original_image` 负责剥离。
  **正文图不做剥离**（`.c1500x.jpg` 是源站定好的画质/体积平衡）。
...
1. **源站是繁体（zh-hant）**，且 `Accept-Language: zh-Hans` 只改界面文案、**不改作品名**：
   ...
   **繁简由项目统一处理**（`zhconv` 繁转简，**只换字形、不做词汇改译**），**适配器不需要自己转换**：
   - **跨源指纹去重**：`fingerprint.build_fingerprint` 的归一化含繁转简 →「電鋸人」与
     「电锯人」（作者一致）命中同一指纹，不会各占一行；
   - **标签归一**：`taxonomy` 查表前也做繁转简 → `格鬥`→`格斗`→规范名「动作」；
     未命中的繁体标签回落简体写法（免得 `靈異`/`灵异` 分成两行）。
   （站点自己的数据也不一致：`electro…` 详情里 `theme` 同时有 `格鬥`(繁) 与 `职场`(简)。）
   ⚠️ **入库标题仍按源站原样（繁体）**，库里显示 `電鋸人` 属正常 —— 折叠只用于判重与标签，不改展示。
   （历史数据若要按新规则对齐指纹/标签，跑 `tools/rebuild_fingerprint.py` + `tools/normalize_tags.py`。）
