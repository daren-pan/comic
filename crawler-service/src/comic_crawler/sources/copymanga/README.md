...
1. **源站是繁体（zh-hant）**，且 `Accept-Language: zh-Hans` 只改界面文案、**不改作品名**：
   ...
   影响两处：
   - **跨源指纹去重**：`build_fingerprint` 只做全角→半角与括号剥离，不做繁简归一，
     所以同一部作品若别的源用简体收录过，这里会被当成**另一部**（独立一行）；
   - **标签归一**：`data/tag_synonyms.json` 是简体词表，`格鬥` 匹配不上 `格斗`。
   （站点自己的数据也不一致：`electro…` 详情里 `theme` 同时有 `格鬥`(繁) 与 `职场`(简)。）
   要彻底解决需引入繁简转换（如 `opencc`/`zhconv`）并在指纹与标签两处接入 —— **尚未做**。
