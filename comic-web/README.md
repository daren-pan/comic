...
漫画聚合阅读平台的**前端实现**（架构方案见 `../docs/architecture.md`，数据来自 `../api-service` 提供的 REST API，内容由 `../crawler-service` 采集服务入库）。
...
| `/search` | 分类浏览 / 搜索 | 顶栏「分类」入口；关键词（标题/作者/标签）+ 分类筛选 + 最新/最热排序 + 分页 |
| `/latest` | 最近更新 | 按更新时间的**卡片网格**（`sort=updated` 分页），卡片右上角带相对时间角标 |
| `/rank` | 排行 | 分类区块（作品数 ≥3）+ 块内按**热度**取前 10（`sort=views` → 后端按 `heat` 倒序），区块滚入视口才懒加载；热度 = 1000 起底 + 浏览×1 + 收藏×2 |
...
- **封面自愈**（2026-09-20 起）：后端「触发转存」跑完后会自动自愈**所点那个源**的封面（结果在 `result.coverHeal`，**不往消息中心写**）；管理台页面顶部另有**「封面自愈 · 指定作品」独立面板**（`POST /api/admin/heal-covers`）——填漫画**名称或 ID**（可多个，逗号/空格/换行分隔，**不允许留空**）→ **强制**回源重抓封面并覆盖，只修封面、不转存正文页，走 `heal` 消息类型在消息中心可见（消息 label 显示所填关键字）。
