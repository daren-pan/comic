...
| `/reader/:comicId/:chapterId` | 在线阅读器 | 双阅读模式（左右滑动/竖排连播）、主题切换、设置面板、章节切换、进度记忆续读、顶栏点击唤醒 |
...
- **阅读器**（架构方案 §4.2）：双阅读模式可切换——
  - **左右滑动**：单张居中、懒加载 + 前后页预加载、点击左右 1/4 区域翻页、`←/→` 键盘翻页；
  - **竖排连播**：整章所有页垂直拼接连续滚动，滚动位置实时推导当前页，`loading="lazy"` 延迟加载；
  - **设置面板（顶栏 ⚙）**：阅读模式（左右滑动/竖排连播）+ 背景主题（深色/浅色）本地持久化（`comic_reader_settings`）；
  - **顶栏交互**：2.6s 自动隐藏，点击屏幕中心或任意位置可再次唤回；
  - 翻页即写入历史（localStorage 模拟用户中心），支持跨端续读；
